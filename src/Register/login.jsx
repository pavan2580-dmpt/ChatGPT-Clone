...
function Login() {
  ...
  async function HandleLogin(event){
    ...
    if(GetEmail.length !=0 && GetEmail.length >5)
    Cookies.setItem("email",GetEmail);
...
<div className="Sign-email">
   <form className='FROm' onSubmit={HandleLogin}>
        <div className="inputBox">
                <input type="email" name="signEmail" id="sign-email"  required onChange={(e)=>{SetGetEmail(e.target.value)}}/>
                <span>Email Address</span>
          </div>

           <input type='submit' className='Sumnit' value={"Continue"}  />
   </form>

    <p>Don't have an account? <span className='span'> <Link to={'/register'} >   Sign up </Link></span></p>
</div>
