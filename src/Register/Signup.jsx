      ...
      58.34a16.41 16.41 0 0 0 -8.3 14.36zm23.19-50 54.92-31.72 54.92 31.7v63.42l-54.92 31.7-54.92-31.7z"></path></svg>
        ...
        <div className="body">
          <h1 className='welcome'>
            ...
            <form action="">
              <input type="email" value={`${Cookies.getItem('email')}`} id="email" placeholder='email id' />
            <div className="inputBox">
                <input type={showPassword ? 'password' : 'text'} name="signEmail" id="sign-email" onChange={(e)=>{SetPassword(e.target.value)}} required />
                <span>Password</span>
                {
                        showPassword ? <PiEyeClosedLight className='icons' size={30} onClick={handleTogglePassword}/> : <PiEyeLight className='icons' size={30} onClick={handleTogglePassword}/>
                }
          </div>
          <p className='p'>Forgot password?</p>
            <input type="submit" value="Continue" className='Sumnit' onClick={PassEmailLogin} />

            </form>
            <p className='dont'>Don't have a account? <span>Sign up</span></p>
